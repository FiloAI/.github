...
test('接受 Codex 当前官方无问题文案并继续整段校验', () => {
  const body = `Codex Review: Didn't find any major issues. Already looking forward to the next diff.
...
Codex can also answer questions or update the PR. Try commenting "@codex address that feedback".
  ...
  const comment = { user: { login: 'chatgpt-codex-connector[bot]' }, body }
  assert.equal(hasCurrentHeadCodexReview({ headOid: HEAD, comments: [comment] }), true)
  assert.equal(hasCurrentHeadCodexReview({
    headOid: HEAD,
    comments: [{ ...comment, body: `${body}\nPlease fix another issue.` }],
  }), false)
})
