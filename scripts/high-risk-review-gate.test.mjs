...
test('高风险改动要求 FiloAI owner 对当前 head 留下批准', () => {
  ...
  const approved = evaluateHighRiskApproval({
    ...
    highRisk: true,
    riskReason: 'path:auth/service.ts',
    reviews: [{ login: 'GaoWeiLiuXD', state: 'APPROVED', commit_id: head }],
  })
  assert.equal(approved.satisfied, true)
})

test('FiloAI owner 自己提交高风险改动时不再要求额外 owner 确认', () => {
  for (const authorLogin of ['zqchris', 'jerboy', 'GaoWeiLiuXD']) {
    ...
    assert.equal(result.evidence, `owner-author:${authorLogin.toLowerCase()}`, authorLogin)
  }
})
