...
const ACTIVE_MERGE_VETO_PATTERN =
  /\b(?:(?:i(?:['’]m|\s+am)|we(?:['’]re|\s+are))\s+)?block(?:ing)?\s+(?:this\s+|the\s+)?(?:merge|pr|pull\s+request)\b|\b(?:(?:i|we)\s+)?veto(?:ed|ing)?\s+(?:this\s+|the\s+)?(?:merge|pr|pull\s+request)\b/i
...
const APPROVAL_CLAUSE_QUALIFIER =
  /\b(?:once|after|when|before|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending|awaiting)\b|\b(?:but\s+)?(?:please\s+)?(?:wait\s+(?:for|until)|hold(?:\s+off)?\s+(?:for|until))\b|(?:一旦|等到|等待|如果|若|除非|直到|只要)|(?:在)?[^。！？!?；;，,\n]{1,40}(?:之后|以前|之前)/iu
