...
const OWNER_VETO_LOGINS = new Set(['zqchris', 'jerboy'])
...
const APPROVAL_NEGATION_PATTERN =
  /(?:尚未|还没|没有|不能|无法|不会|不)(?:批准|确认)[^。！？!\n]{0,16}(?:合并|merge)?|\b(?:(?:i|we)\s+)?(?:(?:have|has|had)\s+not|haven['’]t|hasn['’]t|hadn['’]t)\s+approved?\b|\b(?:(?:i|we)\s+)?(?:(?:have|has|had)\s+not|haven['’]t|hasn['’]t|hadn['’]t)\s+(?:given|provided)\b[^.。！？!?\n]{0,32}\b(?:approval|lgtm)\b|\b(?:(?:i|we)\s+)?(?:cannot|can['’]t|could\s+not|couldn['’]t|will\s+not|won['’]t|do\s+not|don['’]t)\s+(?:approve|(?:consider|regard|treat)[^.。！？!?\n]{0,24}\blgtm\b|(?:give|provide)[^.。！？!?\n]{0,24}\blgtm\b)|\b(?:(?:i|we)\s+)?(?:withdraw|retract)(?:ed|ing|s)?\s+(?:(?:my|our|the)\s+)?lgtm\b/i
...
const STANDALONE_APPROVAL_CONDITION =
  /^(?:after|when|once|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending)\b[^.。！？!?；;\n]{0,100}\b(?:fix(?:es|ed)?|pass(?:es|ed)?|complete(?:s|d)?|resolve(?:s|d)?|sign(?:s|ed|ing)?\s*off|approve(?:s|d)?|succeed(?:s|ed|ing)?|green|ready|safe|healthy|done)\b|^(?:after|when|once|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending)\b[^.。！？!?；;\n]{0,100}\b(?:pr|pull\s+request)\s*#?\d+\b[^.。！？!?；;\n]{0,40}\b(?:merge(?:s|d)?|be\s+merged)\b|\bbefore\s+(?:merge|merging)\b|^(?:如果|若|待|等到|需要先|必须先|先)[^。！？!?；;\n]{0,80}(?:修复|处理|解决|通过|完成|签字|确认|批准|成功|变绿)|(?:修复|处理|解决|通过|完成|签字|确认|批准|成功|变绿)后|\b(?:must|needs?\s+to|has\s+to)\b[^。！？!?；;\n]{0,80}\b(?:fix(?:es|ed)?|pass(?:es|ed)?|complete(?:s|d)?|resolve(?:s|d)?|sign(?:s|ed|ing)?\s*off|approve(?:s|d)?|succeed(?:s|ed|ing)?|green|ready|done|before\s+(?:merge|merging))\b/i
...
const STANDALONE_WAIT_HOLD_CONDITION =
  /^(?:(?:but\s+)?(?:please\s+)?(?:wait\s+(?:for|until)|hold(?:\s+off)?\s+(?:for|until))\b|(?:但)?(?:请)?(?:先等|等待|等到))/iu

const DEPENDENCY_MERGE_CONDITION =
  /^(?:after|when|once|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending)\b[^.。！？!?；;\n]{0,100}\b(?:dependenc(?:y|ies)|prerequisite|upstream(?:\s+(?:pr|pull\s+request|change))?|another\s+(?:pr|pull\s+request)|other\s+(?:pr|pull\s+request)|(?:pr|pull\s+request)\s*#?\d+)\b[^.。！？!?；;\n]{0,40}\b(?:merge(?:s|d)?|land(?:s|ed)?)\b/i

const APPROVAL_CLAUSE_QUALIFIER =
  /\b(?:once|after|when|before|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending|awaiting)\b|\b(?:but\s+)?(?:please\s+)?(?:wait\s+(?:for|until)|hold(?:\s+off)?\s+(?:for|until))\b|(?:一旦|等到|等待|先等|如果|若|除非|直到|只要)|(?:在)?[^。！？!?；;，,\n]{1,40}(?:之后|以前|之前)/iu

const CROSS_PR_PREREQUISITE =
  /^(?:after|when|once|if|unless|until|as\s+long\s+as|so\s+long\s+as|on\s+condition\s+that|provided(?:\s+that)?|providing(?:\s+that)?|assuming(?:\s+that)?|subject\s+to|pending)\b|\bbefore\s+(?:this\s+)?(?:merge|merging)\b|\bbefore\s+(?:this\s+one|ours)\b|\bbefore\s+(?:(?:we|i|you|they|maintainers?|the\s+team)\s+(?:can\s+|may\s+|should\s+|will\s+)?merge\b|(?:we|i|you|they)\s+merge\b|(?:this|the\s+current)\s+(?:pr|pull\s+request)\s+(?:can\s+|may\s+|should\s+|will\s+)?(?:merge|be\s+merged)\b)|\b(?:fixed|resolved|completed|approved|green|ready|done|merged)\s+first\b|^(?:如果|若|待|等到)[^。！？!?；;\n]{0,100}|(?:修复|处理|解决|通过|完成|签字|确认|批准|成功|变绿|合并)后|先[^。！？!?；;\n]{0,80}合并|(?:在)?(?:我们|我|维护者|团队)合并(?:本|当前)?(?:\s*PR)?前|(?:在)(?:本|当前|这个|我们的)(?:\s*PR|拉取请求)?\s*(?:合并)?\s*(?:之前|前)\s*(?:合并)?|才能(?:合并|merge)/i
...
function isPendingReleaseCondition(text, prNumber) {
  ...
  if (!PENDING_CONDITION.test(value)
    ...
    && !STANDALONE_WAIT_HOLD_CONDITION.test(value)
    && !DEPENDENCY_MERGE_CONDITION.test(value)) {
    return false
  }
