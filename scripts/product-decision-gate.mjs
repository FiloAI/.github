...
const REVIEWER_SEPARATE_CONCERN_REJECTION_PATTERN =
  /\b(?:reject(?:s|ed|ing)?|object(?:s|ed|ing)?\s+to|(?:refus(?:e|es|ed|ing)|declin(?:e|es|ed|ing))\s+to\s+(?:accept|agree)|(?:refusal|declination)\s+to\s+(?:accept|agree))\b[^.。！？!?\n]{0,80}\b(?:deferral|trade-?off|scope|out\s+of\s+scope|follow-?up|separate\s+(?:concern|issue|pr))\b/i

const NEGATED_REVIEWER_SEPARATE_CONCERN_REJECTION_PATTERN =
  /\b(?:(?:(?:do|does|did)\s+not|don['’]t|doesn['’]t|didn['’]t|never)\s+(?:reject|object|refuse|decline)|(?:am|is|are)\s+not\s+(?:refusing|declining))\b/i
