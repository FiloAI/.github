...
const REVIEWER_SEPARATE_CONCERN_REJECTION_PATTERN =
  /\b(?:disagree(?:s|d|ing)?(?:\s+(?:that|with))?|reject(?:s|ed|ing)?|object(?:s|ed|ing)?\s+to|(?:refus(?:e|es|ed|ing)|declin(?:e|es|ed|ing))\s+to\s+(?:accept|agree)|(?:refusal|declination)\s+to\s+(?:accept|agree))\b[^.。！？!?\n]{0,80}\b(?:deferral|trade-?off|scope|out\s+of\s+scope|follow-?up|separate\s+(?:concern|issue|pr))\b/i

const NEGATED_REVIEWER_SEPARATE_CONCERN_REJECTION_PATTERN =
  /\b(?:(?:(?:do|does|did)\s+not|don['’]t|doesn['’]t|didn['’]t|never)\s+(?:disagree|reject|object|refuse|decline)|(?:am|is|are)\s+not\s+(?:disagreeing|refusing|declining))\b/i
...
function hasOpaqueDispositionEdit(comment) {
  ...
  return entries.some((entry) => {
    ...
    if (entry.opaque && entryKind
      && entryKind === authorDispositionKind(comment.body)) return true
