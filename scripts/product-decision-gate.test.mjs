...
test('reviewer 明确否定接受时不能因关键词误放行', () => {
  for (const reviewerReply of [
    ...
    'We must not accept this deferral.',
    'We must not agree to this scope decision.',
    "We mustn't approve this product trade-off.",
    'This deferral shall not be accepted.',
    'It must not be accepted as a deferral.',
    'This must not be accepted as a separate concern.',
    '不得接受这个延期决定。',
...
test('reviewer 对 fixed claim 的否定确认继续保留产品取舍门', () => {
  for (const reviewerReply of [
    ...
    'I am unable to confirm this is fixed.',
    'I cannot yet confirm this is fixed.',
    'I am not ready to verify this is resolved.',
    "We aren't able to confirm this has been addressed.",
    'I am not in a position to verify this is resolved.',
    'I should not confirm this is fixed.',
    'We must not verify this is resolved.',
    'I shall not confirm this is addressed.',
    'I would not confirm this is fixed.',
    "We shouldn't verify this is resolved.",
    "I wouldn't confirm this is fixed.",
    'I may be unable to confirm this is fixed.',
    'I might not be ready to verify this is resolved.',
    'We could be unable to confirm this is fixed.',
    'I may be in no position to confirm this is fixed.',
    'I am hesitant to confirm this is fixed.',
    'I am reluctant to verify this is resolved.',
    'We are unwilling to confirm this is fixed.',
    'I am not comfortable confirming this is fixed.',
    '确认仍未修复。',
  ]) {
    const candidate = thread({ authorReply: 'Out of scope; defer to a follow-up PR.' })
    candidate.comments.push({
...
test('promoting 或 raising 的严重度升级进入 P1 产品取舍门', () => {
  for (const severityUpdate of [
    ...
    const candidate = thread({
      severity: 'P2',
      authorReply: 'We have no plans to change this.',
    })
    candidate.comments.push({
...
test('not going to fix 或 address 是明确产品取舍延期', () => {
  for (const authorReply of [
    ...
    'WE WON’T MAKE THIS CHANGE.',
    "We won't implement this change.",
    'We will not apply the requested change.',
    'We are not going to change this.',
    "We're not going to implement that change.",
    'We do not plan to change this.',
    'We have no plans to change this.',
    'We had no plan to adopt the requested change.',
    '我们不会实施这个改动。',
    '我们不打算修改这个行为。',
    '我们没有计划作出所要求的修改。',
    'We postponed fixing this.',
    'We are postponing this fix.',
    'We have postponed addressing the finding.',
    'We have put this fix on hold.',
    'We are putting the requested implementation change on hold.',
    'This behavior change has been postponed.',
    '我们推迟修复这个问题。',
    '我们正在搁置所要求的修改。',
...
test('积极进行中的当前修复不是 no-fix disposition', () => {
  for (const authorReply of [
    ...
    "We won't implement this test change; fixed the behavior.",
    'We are not going to change the fixture; fixed the implementation.',
    'We have no plans to change the documentation; fixed the behavior.',
    'We postponed updating the tests; fixed the behavior.',
    'We are postponing the fixture update; fixed the implementation.',
    'We have put the documentation update on hold; fixed the behavior.',
    '我们推迟更新测试，已经修复实现。',
    '我们不会修改测试夹具，已经修复实现。',
